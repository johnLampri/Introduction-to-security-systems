...
void logattempt(const char* path, int is_action_denied,int access_type){
	...
	fprintf(original_fopen_ret,"%u %s %d-%d-%d %d:%d:%d %d %d \n",uid, truepath, tm.tm_mday, tm.tm_mon+1,tm.tm_year+1970,tm.tm_hour,tm.tm_min,tm.tm_sec, access_type,is_action_denied);

	if (inputfileLength>0){
		int i;
		for(i=0;i<MD5_DIGEST_LENGTH;i++){
			fprintf(original_fopen_ret," %02x \n",md5_output[i]);
		}
	}
	fclose(original_fopen_ret);
